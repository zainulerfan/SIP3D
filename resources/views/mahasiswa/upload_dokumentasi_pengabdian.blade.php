...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            <h4 class="fw-bold mb-1">Upload Dokumentasi Pengabdian</h4>
            <p class="text-muted mb-0">
                Unggah foto / video atau tempelkan link Google Drive dokumentasi pengabdian.
            </p>
        </div>
    ...
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-body p-4">
            <div class="row">
                <div class="col-md-8">
                    <h5 class="fw-bold text-success mb-3">{{ $pengabdian->judul }}</h5>

                    <div class="row">
                        <div class="col-sm-6 mb-2">
                            <span class="text-muted small">Ketua Pengabdian (Dosen):</span>
                            <p class="mb-0 fw-medium">{{ $pengabdian->ketua_pengabdian ?? '-' }}</p>
                        </div>
                        <div class="col-sm-6 mb-2">
                            <span class="text-muted small">Bidang:</span>
                            <p class="mb-0 fw-medium">{{ $pengabdian->bidang ?? '-' }}</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 text-md-end">
                    <div class="mb-2">
                        ...
                        <p class="mb-0 fw-bold fs-5">{{ $pengabdian->tahun ?? '-' }}</p>
                    </div>
                    <div class="mb-2">
                        <span class="text-muted small">Status:</span>
                        <p class="mb-0">
                            <span class="badge {{ $pengabdian->status == 'Aktif' ? 'bg-success' : 'bg-secondary' }} px-3 py-2">
                                {{ $pengabdian->status ?? '-' }}
                            </span>
                        </p>
                    </div>
                    <p class="text-muted small mb-0">
                        Mahasiswa Dok.: {{ $pengabdian->mahasiswa_dokumentasi ?? '-' }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body p-4">
            ...
            <form action="{{ route('mahasiswa.dokumentasi_pengabdian.store', $pengabdian->id) }}"
                method="POST"
